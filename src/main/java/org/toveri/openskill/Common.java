...
import static org.toveri.openskill.Statistics.phiMajor;
import static org.toveri.openskill.Statistics.phiMinor;
...
public class Common {
    ...
    public static double v(double x, double t) {
        ...
        return denom > 0 ? phiMinor(xt) / denom : -xt;
    }
    ...
    public static double vt(double x, double t) {
        ...
        double denom = phiMajor(t - xx) - phiMajor(-t - xx);
        if (denom > 0) {
    ...
    public static double wt(double x, double t) {
        double xx = Math.abs(x);
        double denom = phiMajor(t - xx) - phiMajor(-t - xx);
        return denom > 0 ? ((t - xx) * phiMinor(t - xx) + (t + xx) * phiMinor(-t - xx)) / denom
                + vt(x, t) * vt(x, t)
                : 1.0;
    }
